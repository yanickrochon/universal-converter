...
class UnitDefinition {
  constructor(data) {
    ...
    assert(!data.converters || Array.isArray(data.converters), 'Expected converters to be an array, ' + typeof data.conveters + ' given');
    ...
    this.converters.forEach(converter => {
      assert(converter.params && typeof converter.params === 'object', 'Missing params for converter in definition : ' + this.name);
      assert(typeof converter.convert === 'function', 'Converter must have a convert function in definition : ' + this.name);
      assert(Object.keys(converter.params).every(param => typeof converter.params[param] === 'string'), 'Converter param must be a string in definition : ' + this.name);
      assert.deepStrictEqual(fnArgs(converter.convert).sort(), Object.keys(converter.params).sort(), 'Converter and params do not match in definition : ' + this.name);
    });
    */
  }
  ...
  */
  convert(converter, params) {
    ...
    //const paramsArgs = Object.keys(params).sort();
    const fn = this.conversion.converters[converter]
    const args = fnArgs(fn).map(arg => params[arg]);
