import { buildUnitDef } from "../units";

const distanceDef = buildUnitDef({
  type: "distance",
  base: "meter",
  units: {
    "agate [Adobe]": 0.3048 / 12 / 72,
    "alen [Danish]": 0.6277,
    "alen [Scandinavia]": 0.6,
    "alen [Swedish]": 0.5938,
    angstrom: 1e-10,
    "arms-length": 0.7,
    "arpent [Canada]": 58.47,
    "arpent [France]": 58.471308,
    "arsheen [Russia]": 0.7112,
    "arshin [iran]": 1.04,
    "arshin [iraq]": 74.5,
    "astronomical unit": 149597870691,
    attometer: 1e-18,
    bamboo: 3.2,
    barleycorn: 0.0085,
    "bee space": 0.0065,
    bicron: 1e-12,
    "block [East U.S.]": (0.3048 * 5280) / 20,
    "block [Midwest U.S.]": (0.3048 * 5280) / 16,
    "block [South, West U.S.]": (0.3048 * 5280) / 10,
    bohr: 52.918e-12,
    "braccio [Italy]": 0.7,
    "braza [Argentina]": 1.733,
    "braza [Spain]": 1.67,
    "braza [Texas]": 1.693,
    "button measure": 0.000635,
    "cable [U.S.]": 720 * 0.3048,
    "cable [British]": 608 * 0.3048,
    caliber: 0.3048 / 12,
    "cana (canna, canne)": 2,
    "cape foot": 0.314858,
    "cape inch": 0.314858 / 12,
    "cape rood": 3.778296,
    centimeter: 0.01,
    "chain [Gunter, survey]": 66 * (1200 / 3937),
    "chain [Ramsden, engineer]": 100 * 0.3048,
    "ch'ih [China]": 0.35814,
    "chinese foot": 0.371475,
    "chinese inch": 0.0371475,
    "chinese mile": 557.21,
    "chinese yard": 0.89154,
    "city block [East U.S.]": (0.3048 * 5280) / 20,
    "city block [Midwest U.S.]": (0.3048 * 5280) / 16,
    "city block [South, West U.S.]": (0.3048 * 5280) / 10,
    "click [U.S. military]": 1000,
    cuadra: 84,
    "cuadra [Argentina]": 130,
    "cubit [Egyptian]": 0.45,
    "cubit [Royal Egyptian]": 0.5235,
    "cubit [English]": 0.4572,
    "cubit [Roman]": 0.444,
    cuerda: 21,
    decimeter: 0.1,
    dekameter: 10,
    "didot point": 0.000377,
    digit: 0.019,
    "diraa [Egypt]": 0.58,
    "dong [Vietnam]": (2 + 1 / 3) / 100,
    "douzième [watchmaking]": 0.000188,
    "douzième [print]": 0.00017638888889,
    "dra [Iraq]": 0.745,
    "dra [Russia]": 0.7112,
    "el [Dutch]": 0.69,
    "ell [English]": 1.143,
    "ell [Scotland]": 0.945,
    "elle [Germany]": 0.6,
    "elle [Vienna]": 0.7793,
    em: 0.0042175176,
    "estadio [Portugal]": 261,
    "estadio [Spain]": 174,
    exameter: 1e18,
    "faden [Austria]": 1.8965,
    "faden [Switzerland]": 1.8,
    "fall [English]": 6.858,
    "fall [Scotland]": 5.67,
    fathom: 6 * 0.3048,
    "fathom [ancient]": 1.829,
    "faust [Hungary]": 0.10536,
    "feet [pre-1963 Canada]": 0.325,
    "feet [Egypt]": 0.36,
    "feet [France]": 0.3248406,
    "feet [international, U.S.]": 0.3048,
    "feet [iraq]": 0.316,
    "feet [Netherlands]": 0.28313,
    "feet [Rome]": 0.296,
    "feet [survey]": 1200 / 3937,
    femtometer: 1e-15,
    fermi: 1e-15,
    finger: (0.3048 / 12) * 4.5,
    fingerbreadth: (0.3048 / 12) * 0.75,
    fist: 0.1,
    fod: 0.3141,
    "foot [Egypt]": 0.36,
    "foot [France]": 0.3248406,
    "foot [international, U.S.]": 0.3048,
    "foot [iraq]": 0.316,
    "foot [Netherlands]": 0.28313,
    "foot [Rome]": 0.296,
    "foot [survey]": 1200 / 3937,
    "football field [Canada]": 100.584,
    "football field [U.S.]": 91.44,
    "football field [U.S., complete]": 0.9144 * 120,
    "furlong [international]": 0.3048 * 660,
    "furlong [survey]": (1200 / 3937) * 660,
    "fuss [German]": 0.31608,
    gigameter: 1e9,
    gigaparsec: 30.85678e24,
    "gnat's eye": 0.000125,
    goad: 1.3716,
    gry: 0.000211667,
    "hair's breadth": 0.0001,
    "hand [non-equine]": 0.1016,
    handbreadth: 0.08,
    "hat [Cambodia]": 0.5,
    hectometer: 100,
    heer: 73.152,
    "hiro [Japan]": 6 * 0.303,
    hubble: 9.4605e24,
    "hvat [Croatia]": 1.8965,
    "inch [international, U.S.]": 0.3048 / 12,
    iron: 0.3048 / 12 / 48,
    "ken [Japan]": 6 * 0.303,
    kerat: 0.0286,
    kilofoot: 304.8,
    kilometer: 1000,
    kiloparsec: 3.0856776e19,
    kiloyard: 914.4,
    kind: 0.5,
    "klafter [Austria]": 1.8965,
    "klafter [Switzerland]": 1.8,
    klick: 1000,
    kyu: 0.00025,
    "lap [old]": 402.336,
    "lap [competition]": 400,
    "lap [olympic pool]": 100,
    "league [ancient Celtic]": 2275,
    "league [nautical]": 5556,
    "league [UK nautical]": 5559.552,
    "league [UK]": 4828,
    "league [US statute]": 4828.0417,
    leap: 2.0574,
    legoa: 6174.1,
    legua: 4200,
    "legua [Texas]": 4233.4,
    "legua [Spanish, pre-1568]": 4179.4,
    "legua [Spanish, post-1568]": 6680,
    "li [ancient China]": 500,
    "li [imperial China]": 644.65,
    "li [modern China]": 500,
    "lieue [France]": 3898,
    "lieue [France, metric]": 4000,
    "lieue [France, nautical]": 5556,
    "light second": 299792458,
    "light minute": 299792458 * 60,
    "light hour": 299792458 * 60 * 60,
    "light day": 299792458 * 60 * 60 * 24,
    "light year [Julian]": 299792458 * 60 * 60 * 24 * 365.25,
    "light year [tropical]": 299792458 * 31556925.9747,
    "light year [traditional]": 299792458 * 60 * 60 * 24 * 365,
    "ligne [France]": 0.0021167,
    "ligne [Swiss]": 0.002256,
    line: 0.0021167,
    "line [small]": 0.000635,
    "link [Gunter, survey]": 0.66 * (1200 / 3937),
    "link [Ramden, engineer]": 0.3048,
    lug: 5.0292,
    "lug [great]": 6.4008,
    marathon: 42194.988,
    "mark twain": 3.6576074,
    megameter: 1000000,
    megaparsec: 3.0856776e22,
    "meile [Austria]": 7586,
    "meile [geographische]": 7412.7,
    "meile [North Germany]": 7532.5,
    meter: 1,
    metre: 1,
    "metric mile": 1500,
    "metric mile [high school]": 1600,
    microinch: 2.54e-8,
    micrometer: 0.000001,
    micromicron: 1e-12,
    micron: 0.000001,
    miglio: 1488.6,
    "miil (mijl) [Danish]": 7500,
    "miil (mijl) [Denmark]": 7532.5,
    "miil (mijl) [Sweden, ancient]": 10687,
    mil: 0.0000254,
    "mil [Sweden]": 10000,
    "mile [Britain, ancient]": 1609,
    "mile [Irish]": 2048,
    "mile [international]": 1609.344,
    "mile [nautical, international]": 1852,
    "mile [nautical, UK]": 0.3048 * 6080,
    "mile [nautical, US]": 1852,
    "mile [Roman, ancient]": 1520,
    "mile [Scottish]": 1814,
    "mile [statute]": 1609.344,
    "mile [survey, US]": (1200 / 3937) * 5280,
    "milha [Portuguese]": 2087.3,
    "military pace": 0.762,
    "military pace [double time]": 0.9144,
    "milla [Spanish]": 1392,
    "mille [French]": 1949,
    "milliare [Rome]": 0.001478,
    millimeter: 0.001,
    millimicron: 1e-9,
    "mkono [Africa]": 0.4572,
    "moot [India]": 0.0762,
    myriameter: 10000,
    nail: 0.05715,
    nanometer: 1e-9,
    nanon: 1e-9,
    "pace [great]": 1.524,
    "pace [Roman]": 1.48,
    "palm [Dutch]": 0.1,
    "palm [Britain, Roman minor]": 0.075,
    "palm [US, Roman major]": 0.2286,
    "palmo [Portuguese]": 0.22,
    "palmo [Spanish]": 0.2,
    "palmo [Texas]": 0.2117,
    parasang: 6000,
    "Paris foot": 0.3248406,
    parsec: 149597870691 * 206264.8,
    "pe [Portuguese]": 0.33324,
    pearl: 0.001757299,
    perch: 5.0292,
    "perch [Ireland]": 6.4008,
    pertica: 2.96,
    pes: 0.2967,
    petameter: 1e15,
    pica: 0.0042175176,
    picometer: 1e-12,
    "pie [Argentina]": 0.2889,
    "pie [Italian]": 0.298,
    "pie [Spanish]": 0.2786,
    "pie [Texas]": 0.2822,
    "pied de roi": 0.3248406,
    pik: 0.71,
    "pike [Greece]": 0.71,
    "point [Adobe]": 0.3048 / 12 / 72,
    "point [Britain, US]": 0.00035,
    "point [Didot]": 0.000377,
    "point [TeX]": 0.0003514598035,
    pole: 5.0292,
    "polegada [Portuguese]": 0.02777,
    pouce: 0.02707,
    "pu [China]": 1.7907,
    pulgada: 0.02365,
    "pygme [Greece]": 0.346,
    Q: 0.00025,
    quadrant: 10001300,
    quarter: 402.336,
    "quarter [cloth]": 0.2286,
    "quarter [print]": 0.00025,
    range: (1200 / 3937) * 31680,
    "reed [Israel]": 2.679,
    "ri [Japan]": 3927,
    ridge: 6.1722,
    "river [Egypt]": 2000,
    Robie: 0.001 * 25,
    "rod [international]": 5.0292,
    "rod [survey]": (1200 / 3937) * 16.5,
    roede: 10,
    rood: 3.7783,
    rope: 3.7783,
    "royal foot": 0.3248406,
    "rute [Germany]": 3.75,
    sadzhen: 2.1336,
    sagene: 2.1336,
    "Scots foot": 0.30645,
    "Scots mile": 1814.2,
    seemeile: 1852,
    shackle: 27.432,
    shaftment: 0.15124,
    "shaftment [ancient]": 0.165,
    "shaku [Japan]": 0.303,
    siriometer: 1.4959787e17,
    smoot: 1.7018,
    span: 0.2286,
    spat: 1e12,
    stadium: 185,
    step: 0.762,
    stick: 3.048,
    story: 3.3,
    "stride [great]": 1.524,
    "stride [Roman]": 1.48,
    tenthmeter: 1e-10,
    terameter: 1e12,
    thou: 0.0000254,
    toise: 1.949,
    township: (1200 / 3937) * 31680,
    "t'sun": 0.0358,
    tu: 161130,
    twain: 3.6576074,
    twip: 0.000017639,
    U: 0.04445,
    "vara [California]": 0.83820168,
    "vara [Mexico]": 0.83802,
    "vara [Portuguese]": 1.1,
    "vara [South America]": 0.864,
    "vara [Spanish]": 0.83587,
    "vara [Texas]": 0.84666836,
    verge: 0.9144,
    vershok: 0.04445,
    verst: 1066.8,
    "wah [Thailand]": 2,
    werst: 1066.8,
    "X unit": 1.0020722e-13,
    yard: 0.9144,
    yoctometer: 1e-24,
    yottameter: 1e24,
    zeptometer: 1e-21,
    zettameter: 1e21,
    "zoll [Germany]": 0.02634,
    "zoll [Switzerland]": 0.03,
  },
  aliases: {
    in: "inch [international, U.S.]",
    ft: "feet [international, U.S.]",
    yd: "yard",
    mm: "millimeter",
    cm: "centimeter",
    m: "meter",
    km: "kilometer",
    inch: "inch [international, U.S.]",
    foot: "foot [international, U.S.]",
    feet: "feet [international, U.S.]",
    mile: "mile [international]",
  },
  conversion: {
    params: {
      surface: "area",
      length: "distance",
      initialVelocity: "velocity",
      velocity: "velocity",
      time: "time",
    },
    converters: {
      squareWidth: ({ surface = 0 }) => Math.sqrt(surface),
      rectangleWidth: ({ surface = 0, length = 0 }) => surface / length,
      velocityAndTime: ({ initialVelocity = 0, velocity = 0, time = 0 }) =>
        ((initialVelocity + velocity) * time) / 2,
    },
  },
});

export type DistanceType = typeof distanceDef;
export default distanceDef;
